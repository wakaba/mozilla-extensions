...
function createNewItem()
{
	...
	myDatasource.Assert(RitemId, RDF.GetResource("http://www.xulplanet.com/rdf/prefbar#type"),    RDF.GetLiteral("spacer"), true); 
	myDatasource.Assert(RitemId, RDF.GetResource("http://www.xulplanet.com/rdf/prefbar#label"),   RDF.GetLiteral("(spacer)"), true); 
	myDatasource.Assert(RitemId, RDF.GetResource("http://www.xulplanet.com/rdf/prefbar#enabled"), RDF.GetLiteral("true"), true); 
}

function editItem()
{
	debugMessage("Editing item without any editable attributes. :]");
}